...
class Player(object):
    ...
    def send_self(self):
        self.send_msg(
            msg.S_msg(0, self._roles[0]._role_name, 'player_info', self.to_dict()))
    ...
    def send_msg(self, smsg):
        def send(smsg):
            try:
                ...
                loop.run_until_complete(self.websocket.send_text(json.dumps(
                    smsg, default=lambda o: o.__dict__, ensure_ascii=False)))
    ...
    def create_room(self, rmsg: msg.R_msg) -> msg.S_msg:
        ...
        self.room = room_center.create_room(
            self, rmsg.op_content['roles_config'])
        logging.info('创建房间成功')
    ...
    def vote(self, rmsg: msg.R_msg) -> None:
        if(self.vote_msg is None):
            ...
            self.send_msg(msg.S_msg(
                0, '投票成功:'+self.room.players[openid].nickName, 'info', self.vote_msg))
        else:
            openid = self.vote_msg.op_content['vote_msg']['openid']
            self.send_msg(msg.S_msg(
                0, '已经投过票:'+self.room.players[openid].nickName, 'info', self.vote_msg))
        self.room.end_game()
