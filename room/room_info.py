...
class GameRoom(object):
    ...
    def cal_game(self):
        ...
        if flag:
            ...
            time.sleep(5)
            for player in self.players.values():
                player.send_msg(msg.S_msg(0, f'游戏开始，请从{start}号开始发言......', 'info', None))
        return None
    ...
    def end_game(self):
        ...
        '''通知猎人发动技能'''
        if hunter != None:
            hunter.send_msg(msg.S_msg(0, 'hunter', 'wait_back', {'hunter':hunter.to_dict()}))
        else:
            self.game_result()
        return None
    ...
    def game_result(self):
        ...
        if self._game_end_time is None:
            ...
            logging.info(f'房间[{self.roomid}]游戏结束villager_win:[{villager_win}], [{self.to_dict()}]')
